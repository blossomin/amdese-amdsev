...
if [ "${SEV_SNP}" = 1 ]; then
    ...
    add_opts "-drive if=pflash,format=raw,unit=0,file=${UEFI_VARS},readonly=on"
else
...
if [ ${SEV} = "1" ]; then
	...
	if [ "${SEV_SNP}" = 1 ]; then
		...
		if [ "${CERTS_PATH}" != "" ]; then
			add_opts "-object sev-snp-guest,id=sev0,policy=${POLICY},cbitpos=${CBITPOS},reduced-phys-bits=1,kernel-hashes=on,certs-path=${CERTS_PATH}"
		else
			add_opts "-object sev-snp-guest,id=sev0,policy=${POLICY},cbitpos=${CBITPOS},reduced-phys-bits=1,kernel-hashes=on"
		fi
	else
		POLICY=$((0x01))
